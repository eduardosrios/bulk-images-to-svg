(function () {
  ...
  els.duardoLink = document.querySelector(".duardo-link");
  els.duardoMeta = document.querySelector(".duardo-meta");
  ...
  function initDuarDoFooter() {
    if (!els.duardoLink) return;
    initDuarDoAnimation();
    bindResponsiveDuarDoBrand();
  }

  function bindResponsiveDuarDoBrand() {
    let resizeTimer = 0;

    window.addEventListener("resize", function () {
      window.clearTimeout(resizeTimer);
      resizeTimer = window.setTimeout(fitDuarDoBrand, 80);
    });

    if (document.fonts && document.fonts.ready) {
      document.fonts.ready.then(queueDuarDoFitBurst);
    }

    window.addEventListener("load", queueDuarDoFitBurst, { once: true });
    bindDuarDoFirstInteractionFit();
    queueDuarDoFitBurst();
  }

  function bindDuarDoFirstInteractionFit() {
    const events = ["pointerover", "pointerdown", "wheel", "scroll", "keydown", "touchstart"];
    let hasRun = false;
  ...
  function initDuarDoAnimation() {
    const link = els.duardoLink;
    const reduceMotion = window.matchMedia("(prefers-reduced-motion: reduce)");
    let letters;
    let text;

    if (!link || link.dataset.animated === "true") return;

    text = link.textContent.trim();
    link.setAttribute("aria-label", text);
    link.dataset.animated = "true";
    link.innerHTML = text.split("").map(function (char, index) {
      return `<span class="duardo-letter" aria-hidden="true" data-letter-index="${index}">${escapeHtml(char)}</span>`;
    }).join("");

    if (reduceMotion.matches || !window.gsap) return;

    letters = Array.prototype.slice.call(link.querySelectorAll(".duardo-letter"));

    window.gsap.fromTo(letters, {
      yPercent: 120,
      rotateX: -72,
      opacity: 0
    }, {
    ...
    letters.forEach(function (letter, index) {
      letter.addEventListener("mouseenter", function () {
        animateDuarDoLetter(letter, index);
      });
  ...
  function animateDuarDoLetter(letter, index) {
    const moves = [
      { rotationX: -180, yPercent: -18, xPercent: 0 },
      { rotationX: 180, yPercent: 18, xPercent: 0 },
      { rotationY: -180, xPercent: -12, yPercent: 0 },
      { rotationY: 180, xPercent: 12, yPercent: 0 }
    ];
    let move;

    if (!window.gsap) return;
    window.gsap.killTweensOf(letter);

    if (index === 0) {
      window.gsap.timeline()
        .to(letter, { xPercent: -18, skewX: -12, scaleX: 1.14, duration: 0.16, ease: "power2.out" })
        .to(letter, { xPercent: 14, skewX: 9, scaleX: 0.92, duration: 0.2, ease: "power3.inOut" })
        .to(letter, { xPercent: 0, skewX: 0, scaleX: 1, duration: 0.48, ease: "elastic.out(1, 0.36)" });
      return;
    }

    if (index === 2) {
      window.gsap.timeline()
        .to(letter, { rotation: 360, yPercent: -8, duration: 0.54, ease: "back.inOut(1.45)" })
        .to(letter, { rotation: 0, yPercent: 0, duration: 0.18, ease: "power2.out" });
      return;
    }

    if (index === 5) {
      window.gsap.timeline()
        .to(letter, { z: -180, scale: 0.56, opacity: 0.76, duration: 0.26, ease: "power3.out" })
        .to(letter, { z: 0, scale: 1, opacity: 1, duration: 0.5, ease: "elastic.out(1, 0.42)" })
        .set(letter, { clearProps: "z,scale,opacity" });
      return;
    }
    ...
    window.gsap.timeline()
      .to(letter, {
        rotationX: move.rotationX || 0,
        rotationY: move.rotationY || 0,
        xPercent: move.xPercent,
        yPercent: move.yPercent,
        duration: 0.28,
        ease: "power2.out"
      })
      .to(letter, {
  ...
  function queueDuarDoFit() {
    if (!els.duardoLink) return;
    window.requestAnimationFrame(fitDuarDoBrand);
  }

  function queueDuarDoFitBurst() {
    queueDuarDoFit();
    window.setTimeout(queueDuarDoFit, 80);
    window.setTimeout(queueDuarDoFit, 260);
  }

  function fitDuarDoBrand() {
    const link = els.duardoLink;
    let maxWidth;
    let low = 24;
    let high;
    ...
    maxWidth = link.clientWidth;
    if (!maxWidth) return;

    high = Math.max(80, Math.min(420, maxWidth));
    link.style.fontSize = `${low}px`;
  ...
  function syncDuarDoMetaWidth() {
    const link = els.duardoLink;
    const meta = els.duardoMeta;
    const width = link && meta ? Math.max(0, Math.round(getDuarDoTextWidth(link) - 20)) : 0;
    if (width > 0) meta.style.setProperty("--duardo-meta-width", `${width}px`);
  }

  function getDuarDoTextWidth(link) {
    const letters = Array.prototype.slice.call(link.querySelectorAll(".duardo-letter"));
    let range;
    let rect;

    if (letters.length) {
      return letters[letters.length - 1].offsetLeft +
        letters[letters.length - 1].offsetWidth -
